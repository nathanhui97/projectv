...
A mobile TCG simulator for a popular collectible card game. Built solo, AI-assisted, ship-quality.
...
9. **[09-mobile-app.md](09-mobile-app.md)** — what users see and touch
...
- **Hot-updatable by default.** Anything that can be changed without an app release should be. App store reviews are slow; player frustration with stale data is fast.
...
- **Engine logic is shared.** The rules engine is TypeScript that runs identically on both clients in a match. The mobile app is a thin renderer over it.
...
This is a monorepo. The shared `engine` and `schemas` packages are the spine — the mobile app, admin portal, and any future server all depend on them.
